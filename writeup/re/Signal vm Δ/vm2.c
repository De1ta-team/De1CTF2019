...
//        printf("0x%llx  ",reg.rip);
        *(long*)peek_data = ptrace(PTRACE_PEEKTEXT,a,reg.rip,0);
        if (status[1] == 5)
        {
            *(long long*)peek_data = ptrace(PTRACE_PEEKTEXT,a,reg.rip-1,0);
//          puts("SIGILL,  mov");
            type = peek_data[1];
            op0 = (int)peek_data[2];
            if(type == 0x01)
            ...
            switch(type)
            {
                case 0x00:
                    addr1 = &R[op1];
                    tmp = ptrace(PTRACE_PEEKTEXT,a,addr1,0);
                    addr0 = &R[op0];
                    ptrace(PTRACE_POKETEXT, a, addr0,tmp);
                    //R[op0] = R[op1];
                    //printf("SIGILL,  mov R%d, R%d\n",op0,op1);
                    break;
                case 0x01:
                    addr0 = (void*)&R[op0];
                    //tmps = ptrace(PTRACE_PEEKTEXT,a,addr0,0);
                    //printf("%llx ",tmps);
                    ptrace(PTRACE_POKETEXT, a, addr0,op1);
                    //R[op0] = op1
                ...
                case 0x20:
                    ...
                    addr0 = &memory[tmps];
                    addr1 = &R[op1];
                    tmpchar = ptrace(PTRACE_PEEKTEXT,a,addr1,0)&0xFF;
                    set_byte(a,addr0,tmpchar);
                    addr1 = &memory[98];
                    //memory[R[op0]] = R[op1];
                ...
                case 0x02:
                    addr0 = &R[op0];
                    addr1 = &R[op1];
                    tmps = ptrace(PTRACE_PEEKTEXT,a,addr1,0);
                    addr1 = &memory[tmps];
                    tmpchar = ptrace(PTRACE_PEEKTEXT,a,addr1,0)&0xFF;
                    tmp = (unsigned long long)tmpchar;
                    ptrace(PTRACE_POKETEXT,a,addr0,tmp);
                    //R[op0] = memory[R[op1]];
        ...
        {
            *(long*)peek_data = ptrace(PTRACE_PEEKTEXT,a,reg.rip+1,0);
//            puts("SIGTRAP, opt");
            op_type = peek_data[0];
            type = peek_data[1];
            op0 = peek_data[2];
            addr0 = &R[op0];
            tmp = ptrace(PTRACE_PEEKTEXT,a,addr0,0);
            if(type == 0x01)
            {
                reg.rip+=8;
                op1 = *(int*)&peek_data[3];
            ...
            else if(type == 0x00)
            {
                reg.rip+=5;
                addr1 = &R[peek_data[3]];
                op1 = (int)ptrace(PTRACE_PEEKTEXT,a,addr1,0);
...
//            puts("SIGFPE,  cmp");
            ...
            else if(type == 0x00)
            {
                op1 = (int)peek_data[4];
                addr1 = &R[op0];
                tmp1 = ptrace(PTRACE_PEEKTEXT,a,addr1,0);
                addr0 = &R[op1];
                tmp = ptrace(PTRACE_PEEKTEXT,a,addr0,0);
                tmp = tmp1-tmp;
                addr0 = &R[9];
...
void show()
{
    puts("               _              __________   _____________   ___________");
    puts("              / \\           / _________| |_____   _____| |  _________|");
    puts("             / _ \\         | /                 | |       | |");
    puts("            / / \\ \\        | |                 | |       | |");
    puts("           / / _ \\ \\       | |                 | |       | |_______");
    puts("          / / / \\ \\ \\      | |                 | |       |  _______|");
    ...
    puts("        / / / /_\\ \\ \\ \\    | |                 | |       | |");
    puts("       / / /_______\\ \\ \\   | |                 | |       | |");
    puts("      / /_____________\\ \\  | \\_________        | |       | |");
    puts("     /___________________\\  \\__________|       |_|       |_|");
    puts(" ");
    puts(" ");
    puts("Welcome to De1CTF.");
    puts("Congratulation! You can get a free flag!");
